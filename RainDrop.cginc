									...
									float3 viewDir,float3x3 tanToWorld)
{
    ...
    float3 constDir=float3(-0.3,1,-0.3);
    constDir=normalize(constDir);
    viewDir=normalize(viewDir+constDir);
    ...
    float RdotV=smoothstep(0.9,1.2,dot(riseNormal,viewDir));
    RdotV*=RdotV;
    ...
    float DdotC=smoothstep(0.5,1.5,dot(downNormal,constDir));
    DdotC*=DdotC;
